...
sed -e "s/\${Pro4K}/"$Pro4K"/" -e "s/\${ProHD}/"$ProHD"/"  -e "s/\${Hq4K}/"$Hq4K"/" -e "s/\${HqHD}/"$HqHD"/" -e "s/\${HqVGA}/"$HqVGA"/" /mnt/mmc/scripts/init.tp >  /mnt/mmc/scripts/init.sh
sync;sync;sync
/mnt/mmc/scripts/popup_timeout  " [ Bitrates Updated  ] " 2
killall mod_gui
