...
sed -e "s/\${Pro4K}/"$Pro4K"/" -e "s/\${ProHD}/"$ProHD"/"  -e "s/\${Hq4K}/"$Hq4K"/" -e "s/\${HqHD}/"$HqHD"/" -e "s/\${HqVGA}/"$HqVGA"/"  -e "s/\${fps}/"$fps"/"  /opt/home/scripts/init_auto.tp >  /opt/home/scripts/auto/init.sh
sync;sync;sync
exit
