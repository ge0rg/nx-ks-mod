...
for i in "${tii[@]}"; do if [[ $i == "swmo="* ]]; then swap=${i:5}; fi; done

sed -e "s/\${Pro4K}/"$Pro4K"/" -e "s/\${ProHD}/"$ProHD"/"  -e "s/\${Hq4K}/"$Hq4K"/" -e "s/\${HqHD}/"$HqHD"/" -e "s/\${HqVGA}/"$HqVGA"/"  -e "s/\${swap}/"$swap"/" /opt/home/scripts/init_auto.tp >  /opt/home/scripts/auto/init.sh
sync;sync;sync
exit
