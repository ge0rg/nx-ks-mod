...
[[  "$cb" == "" ]] && /opt/usr/nx-ks/popup_timeout  " [  Profile Not Adjustable  ] " 2 && exit
br=$(/opt/usr/nx-ks/popup_entry "Current Bitrate ( Mbps ) :" "Apply Change" Cancel $cb number )
[[ "$br" == "" ]]  && exit
[[ "${avail_br[@]}" =~ "${br}" ]] ||  { $(/opt/usr/nx-ks/popup_timeout  " [  Value Not Available  ] " 3) &&  exit ; }
...
/opt/usr/nx-ks/popup_timeout  " [ Current Bitrate Updated  ] " 2
