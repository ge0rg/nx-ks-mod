...
#
tl_d=$(/opt/usr/nx-ks/popup_entry "Time-lapse duration:" "Set Minutes" Cancel 30 number )
[[ $tl_d =~ ^[0-9]+$ ]] || exit
tl_d=$(($tl_d*60))
#
tl_g=$(/opt/usr/nx-ks/popup_entry  "Time between images :" "Set Seconds" Cancel 5 number)
[[ $tl_g =~ ^[0-9]+$ ]] || exit
#
sleepytime=$(/opt/usr/nx-ks/popup_entry "Start Time in :" "Set Minutes and START" Cancel 0 number )
[[ $sleepytime =~ ^[0-9]+$ ]] || exit
#
